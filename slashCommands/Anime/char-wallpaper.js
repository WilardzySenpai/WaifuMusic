...
module.exports = {
    ...
    options: [
        {
            name: 'char',
            description: 'Provide a anime character name',
            type: 3,
            required: false
        }
    ], // options string
    execute: async (client, interaction) => {
        wbc.send(`[slashCommand] :: **Char-wallpaper used by ${interaction.user.tag} from ${interaction.guild.name}**`);
        ...
        try {
            if (!query) return interaction.reply({ content: client.emoji.cross + " | Please provide a character name" })
            async function Wallpaper5() {
                const wallpaper = await wall.getAnimeWall5(`${query}`)
                return wallpaper;
            }
            try {
                var wallpapers = await Wallpaper5();
            } catch (e) {
                console.log(e)
            ...
            const embed = new EmbedBuilder()
                ...
                .setFooter({ text: `HD ${query} Wallpaper!` })
